...
//批量获取服务器用户资料
RCT_EXPORT_METHOD(batchFetchUserInfo:(nonnull NSArray *)userIds resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject){
    [[ContactViewController initWithContactViewController]batchfetchUserInfos:userIds Success:^(id users) {
        resolve(users);
    } error:^(NSString *error) {
        reject(@"-1",error, nil);
    }];
...
// 根据消息id查询消息记录
RCT_EXPORT_METHOD(queryMessageById:(nonnull NSString *)messageId resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject) {
    [[ConversationViewController initWithConversationViewController]localSession:messageId success:^(id data) {
        resolve(data);
    } err:^(id err) {
        reject(@"-1",err, nil);
    }];
...
//更新自定义消息
RCT_EXPORT_METHOD(updateCustomMessage:(nonnull NSString *)messageId andAttachment:(nonnull  NSDictionary *)attachment){
    [[ConversationViewController initWithConversationViewController]updateCustomMessage:messageId andAttachment:attachment];
}
//发送视频消息
RCT_EXPORT_METHOD(sendVideoMessage:(nonnull  NSString *)file duration:(NSString *)duration width:(NSInteger)width height:(NSInteger)height displayName:(NSString *)displayName){
...
//将所有联系人的未读数清零
RCT_EXPORT_METHOD(allMessagesRead){
