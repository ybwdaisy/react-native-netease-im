...
-(void)getRecentContactListsuccess:(SUCCESS)suc andError:(ERROR)err{
    ...
    for (NIMRecentSession *recent in NIMlistArr) {
        ...
        // 扩展字段
        if ([self extForRecentSession:recent.session.sessionId] != nil) {
            [dic setObject:[self extForRecentSession:recent.session.sessionId] forKey:@"ext"];
        }
...
// 扩展字段
-(NSString *)extForRecentSession:(NSString *)sessionId{
