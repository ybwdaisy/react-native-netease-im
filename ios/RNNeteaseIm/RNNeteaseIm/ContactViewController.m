...
//批量获取服务器用户资料
-(void)batchfetchUserInfos:(NSArray *)userIds Success:(Success )success error:(Error )err{
    [[NIMSDK sharedSDK].userManager fetchUserInfos:userIds completion:^(NSArray *users, NSError *error) {
        if (users.count) {
            NSMutableArray *userDictArr = [NSMutableArray arrayWithCapacity:0];
            for (NIMUser *user in users) {
                NSMutableDictionary *userDict = [self setupUserDict:user andUserId:user.userId];
                [userDictArr addObject:userDict];
            }
            success(userDictArr);
        } else {
            err(@"用户账号为空");
